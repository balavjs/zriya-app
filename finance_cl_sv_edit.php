<?php
  $title = "Finance Clothing - Sweden Update | Zriya Solutions";
  ob_start();
?>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"><a href="finance_cl_sv_view.php">Finance Clothing - Sweden</a></li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">             
            ...
            <div class="card">
              <div class="card-header card-zg">
                <h3 class="card-title">Finance Clothing - Sweden Details</h3>
              </div>        
              ...
              if(isset($_POST['update'])){
                ...
                $sql = $result->update($id, $id1, $signed, $inv_date, $terms, $due_date, $comp_id, $p_status, $p_date, $p_comment, $product_desc, $u_price, $qty, $price, $vat, $total, $pricetotal, $momtotal, $subtotal, $advance, $gtotal, $comment, $status);
              ...
              ?>                
                ...
                <form method="post">
                  <?php
                    $id = $_GET['id'];
                    $result1 = new DB_finance_cl_sv();
                    $sql1 = $result1->get_one_finance_cl_sv($id);   
                  ...
                  <div class="row">
                    <table class="table" id="orders">  
                      ...
                      $result=new DB_finance_cl_sv();
                      $id = $_GET['id'];
                      $sqlf = $result->list_finance_cl_sv_desc($id);
                      $i=0;
                      ...
                      foreach ($sqlf as $list) {  
                        ...
                        $id1 = $list['id'];
                        $finance_cl_sv_id     = $list['finance_cl_sv_id'];       
                        $product_desc = $list['product_desc'];
                        $u_price      = $list['u_price'];
                        $qty          = $list['qty'];
                        $price        = $list['price'];   
                        $vat          = $list['vat']; 
                        $total        = $list['total'];                     
                      ...
                      ?>
                      <tr id="tr_rows">
                        <input type="hidden" class="form-control" name="finance_cl_sv_id[]" value="<?php echo $finance_cl_sv_id; ?>">  
                        <input type="hidden" class="form-control" name="id1[]" value="<?php echo $id1; ?>">
                        <td><input type="text" class="form-control" name="product_desc[]" placeholder="Description" id="description_<?php echo $i; ?>" data-type="product_desc" value="<?php echo $product_desc; ?>" for="<?php echo $i; ?>"/ required></td>
                        <td><input class="form-control u_price" type='number' data-type="u_price" id='u_price_<?php echo $i; ?>' name='u_price[]' value="<?php echo $u_price; ?>" for="<?php echo $i; ?>"/ min="0" step="any" required></td>
                        <td><input class="form-control qty" type='number' data-type="qty" id='qty_<?php echo $i; ?>' name='qty[]' value="<?php echo $qty; ?>" for="<?php echo $i; ?>"/ min="0" step="any" required></td>
                        <td><input class="form-control price" type='number' data-type="price" id='price_<?php echo $i; ?>' name='price[]' value="<?php echo $price; ?>" for="<?php echo $i; ?>"/ step="any" readonly></td> 
                        ...
                        <td><a href="finance_cl_sv_det_delete.php?id=<?php echo $id1;?>&row_id=<?php echo $id;?>" onClick="return confirm('Do you really want to delete?');"><button type="button" name="remove" id="<?php echo $i; ?>" class="btn btn-danger btn_remove cicle">-</button></a></td>                 
                      </tr>
                      <?php
                    }
                  }
...
function getTotalCost(ind) {
  var u_price = $('#u_price_'+ind).val();
  var qty = $('#qty_'+ind).val();
  var pricetot = (u_price * qty).toFixed(2);
  $('#price_'+ind).val(pricetot);
...
function calculateSubTotal1() {
  ...
  var subtotal = 0; 
  $('.total').each(function() {
     subtotal += parseFloat($(this).val());1
  });
  $('.price').each(function() {
     pricetotal += parseFloat($(this).val());
  });
  ...
  var pricetotal = pricetotal.toFixed(2);
  var momtotal = momtotal.toFixed(2);
  var subtotal = subtotal.toFixed(2);1
  $('#pricetotal').val(pricetotal);
  $('#momtotal').val(momtotal);
  $('#subtotal').val(subtotal);
